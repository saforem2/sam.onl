...
# 2. venv on top: uv, pinned to the module's python so it inherits torch/MPI
uv venv --system-site-packages --python python3 "venvs/$(basename $CONDA_PREFIX)"
...
prebuilt torch instead of pulling a fresh CPU-only wheel; naming it after
`$CONDA_PREFIX` keeps one venv per frameworks release so a module bump does not
silently mix ABIs. One `uv venv` gotcha: it defaults to a uv-managed CPython
...
flowchart LR
    ...
    class A1,A2,A8 g1
    class A3 dots
    class B1,B2,B3,B4 g2
```

_Each finished node becomes a source for up to 8 more → the tree grows `O(log N)`._
...
The fan-out diagram (from the yeet write-up): a single source only pushes to ~8
nodes before its NIC saturates, so each node that finishes becomes a source for
the next batch. The tree grows recursively, so wall-clock is ~O(log N), not O(N).
</SpeakerNotes>

<style>{`
  /* Compact fan-out tree under the two columns; cap it so the sm slide fits. */
  #yeet .mermaid svg { width: 100%; max-width: 100%; height: auto; max-height: 11lh; }
  #yeet .mermaid { margin: 0.3lh 0 0; }
  #yeet .slide-inner > p:last-of-type { font-size: 0.82em; color: var(--foreground2, #838383); margin-block-start: 0.3lh; }
`}</style>
...
<style>{`
  /* The B_crit schematic sits under the definition line, above the two
     columns. Cap it so the dense slide (2 cols + tip) still fits. */
  #critical-batch-size .cbs-fig { margin: 0.3lh 0 0.5lh; }
  #critical-batch-size .cbs-fig img { width: 100%; height: auto; max-height: 12lh; }
  @media (max-width: 768px) {
...
still looks fine. The next two slides are exactly those.
</SpeakerNotes>
...
run. The next slide is the 20B, same story one scale up.
</SpeakerNotes>
...
{/* source: net-new · results payoff; loss from docs/experiments/agpt/aurora + eval from docs/evals/agpt/20b */}
<Slide id="results-payoff">

## Same story, one scale up: 20B

The **20B** on Aurora, ~600B tokens: loss falls the same way, and the eval gate
moves, well above random on every benchmark.

<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-08-03/figures/results-payoff-mobile.svg"
        />
        <img
            src="/talks/2026-08-03/figures/results-payoff.svg"
            alt="Two panels for AuroraGPT-20B: left, pretraining loss falling from 12.9 to 2.44 over ~605B tokens; right, final lm-eval scores (HellaSwag 0.61, ARC-Easy 0.65, ARC-Challenge 0.36, Winogrande 0.58, PIQA 0.76, BoolQ 0.58) all above their random baselines."
        />
    </picture>
</figure>

<SpeakerNotes>
The one results slide in a how-to talk, placed right before the capstone so the
machinery has a payoff. Everything in Parts 1 and 2, the data pipeline, the
portable launch, the parallelism choices, the failure recovery, exists to get a
model that trains cleanly and learns. This is AuroraGPT-20B on Aurora, the v2
(fp32-master) run, about 600B tokens.

Left panel: pretraining loss falls from 12.9 at init to about 2.44 over ~605B
tokens (the full v2 512N chain, stitched across all its restarts). Right panel:
final lm-eval, every benchmark clears its random baseline (the red dashed line,
0.25 for the 4-way ARC/HellaSwag tasks, 0.5 for the binary Winogrande/PIQA/BoolQ)
by a wide margin: HellaSwag 0.61, ARC-Easy 0.65, ARC-Challenge 0.36, Winogrande
0.58, PIQA 0.76, BoolQ 0.58. Post-training (CPT, SFT, GRPO) and the full
per-benchmark eval curves are in the backup slides and the July AuroraGPT deck.
Data from the ezpz production docs.
</SpeakerNotes>

<style>{`
  #results-payoff figure { margin: 0.4lh 0 0; }
  #results-payoff img { width: 100%; height: auto; max-height: 26lh; }
`}</style>
