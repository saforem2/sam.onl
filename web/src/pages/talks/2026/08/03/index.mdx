...
<figure>
    <picture>
        ...
        <img
            ...
            alt="ZeRO memory partitioning: rows Baseline / P_os / P_os+g / P_os+g+p, columns gpu_0 / gpu_i / gpu_N-1. Each stage shards one more of parameters, gradients, optimizer states from a full-width replicated strip into a narrow 1/N per-GPU column, so per-GPU memory shrinks down the stages."
        />
    </picture>
</figure>
...
<style>{`
  ...
  #zero-fsdp img { width: 100%; height: auto; max-height: 26lh; }
`}</style>
...
- **LR scaling** (linear / $\sqrt{B}$)[^lb-openai] + **longer warmup** + grad clip
...
- **Shampoo, SOAP, Muon are one family:** precondition with second-moment
  structure, differing only in **how cheaply** they approximate it. **SOAP** =
  Adam in Shampoo's eigenbasis (fewer knobs, refresh every $k$ steps); **Muon** =
  the cheaper cousin, Newton-Schulz instead of an eigendecomposition.

<SpeakerNotes>
AdamW is the baseline; everything else buys curvature, a better preconditioner, at
some compute price. Down the table: LAMB is Adam plus a layer-wise trust ratio on
the LR to unlock huge batches. Adopt drops the current grad from the second moment
so it converges for any beta_2. Sophia is a cheap clipped Hessian-diagonal
preconditioner, light curvature. Shampoo, SOAP, and Muon are the structured/spectral
family.

Zoom in on SOAP since it is the cleanest to explain. Shampoo is preconditioned
...
cheaper cousin in the same family: it approximates the same spectral update with
Newton-Schulz orthogonalization instead of an explicit eigendecomposition, skipping
the basis machinery entirely. mano, next, chases the same quality-per-flop from the
opposite direction: manifold normalization instead of eigenbases.
</SpeakerNotes>
...
| 20B save | Sync | Async |
| --- | ---: | ---: |
| training-thread stall | **~23.6 s** | **~5.4 s** (stage 1.7 + drain 3.7) |

</div>

Async cuts the per-save stall **~4.4x** (~18 s back per save).
