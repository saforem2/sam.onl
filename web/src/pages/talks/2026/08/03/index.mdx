...
- **flat/high** at small LR: steps too tiny to learn
- **basin**: the usable band
- **cliff**: one step too big → grads to `inf` → `NaN`
- **pick just below the cliff**, not at the minimum: the
  minimum has no safety margin as the run heats up
...
<style>{`
  ...
  @media (max-width: 768px) { #lr-finder .cols { gap: 1lh; } }
`}</style>
