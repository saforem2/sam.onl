...
- 2x batch ≈ **half the steps** to target loss
- more nodes → less wall-clock, nearly free

**Above $B_{\text{crit}}$**

- speedup **saturates**: more compute, _same_ loss
- efficiency drops, training gets **unstable**[^cbs-zhang][^cbs-zhou]

</Col>
<Col>

**Why HPC hits this first**

- INCITE jobs want **≥ 20% of the machine** (~2k nodes)
- pure DP at that scale → batch past $B_{\text{crit}}$
- fix is not "more DP": TP/PP/EP + a batch you _chose_
- it's a **moving target you measure**, not assume
...
> opposite ways. The rest of the talk lives in that gap.
...
## Large batches: the playbook, and where it breaks
...
- **LR scaling** with batch (linear or $\sqrt{B}$)[^lb-openai]
- **Longer warmup** as the batch grows
- **Stability aids:** grad clip, LAMB, sane $\varepsilon$
...
- past $B_{\text{crit}}$: **efficiency degrades**
...
> [!TIP]+
> **Re-tune at the target batch and node count.** Don't extrapolate a small-batch
> LR up a 100x jump and hope.

<SpeakerNotes>
The standard knobs for large batches: raise the LR with the batch (linear or
sqrt-B) so each step still moves the weights meaningfully; use longer warmup as
the batch grows to survive the noisy early steps before statistics settle; and
lean on stability aids like grad clipping, a large-batch-tuned optimizer such as
LAMB, and a decent epsilon.

What actually breaks: past B_crit sample efficiency degrades, you pay more tokens
for the same loss. Linear LR scaling backfires at the extreme; at 80B, LR-scaling
the 16x batch moved the NaN earlier, step 7 vs step 29. And LRs do not transfer
across batch: mano tuned at GBS=48 loses to AdamW at GBS=384 (2.88 vs 2.71) until
re-tuned. Practical rule: re-tune at the target batch and node count.
</SpeakerNotes>
...
Shampoo finds a good basis, then you do the cheap thing (Adam) inside it.[^soap-paper]
...
- Shampoo's preconditioner → a **rotated frame** (eigenbasis)
- run **plain Adam inside that frame**
- refresh the basis **every $k$ steps** → eigendecomp **amortized**
...
- **fewer knobs:** ~AdamW + basis-refresh frequency
- **more stable:** Adam normalization tames Shampoo
- **keeps the win** at ~Adam per-step cost

</Col>
</Cols>

- **vs Muon:** the cheaper cousin, Newton-Schulz orthogonalization instead of an
  eigendecomposition, skips the basis machinery.[^soap-muon]
...
Kronecker-factored second-moment estimate (left factor from GG^T, right factor
from G^T G) and steps along the preconditioned gradient. Powerful, but the
eigendecompositions and extra knobs (grafting, epsilon, preconditioning
power/frequency) make it expensive and fiddly at scale. SOAP's win is fewer
hyperparameters (essentially AdamW plus the eigenbasis-update frequency), more
stability (Adam's normalization tames Shampoo's rough edges), and it keeps the
preconditioning benefit at close to Adam's per-step cost.
...
Lustre is shared by the whole facility. Every file op is a small tax, paid by every
rank.
...
- **metadata:** `rsync` `stat()`s 10K tiny venv files
- **contention:** you share MDS with every other job
- **fan-out:** one source saturates at ~8 targets → `O(N)`
...
- **stage to node-local `/tmp`** (`ezpz yeet`)
- **one tarball, not 10K files** (`--compress`)
- **coordinate checkpoint writes** (→ async)
- inputs/outputs shared, hot reads local

</Col>
</Cols>

> [!NOTE]+ Measured on Aurora
> Extract-bound at 8-64 nodes (~70-91s flat), broadcast-bound past 128. An
> `O(log N)` fan-out tree, not a star, keeps it tractable to 4,096 nodes.

<SpeakerNotes>
Lustre is shared by the whole facility and your job is one of many tenants. Every
file operation is a small tax, and at scale a small tax paid by every rank turns
into minutes of dead time before training even starts.

How it bites: metadata pressure, since rsync stat()s every file and a venv is tens
of thousands of tiny files, so the first copy is slow because of metadata
round-trips, not bytes. Contention, since you share bandwidth and metadata servers
with everyone else, so your "fast" copy is at the mercy of their job. And fan-out
limits, since a single source saturates its NIC at about 8 targets, so naive
broadcast to thousands of nodes is O(N).

Rules of the road: stage to node-local /tmp with ezpz yeet, which copies the venv
(or dataset, or checkpoint) once so imports hit local SSD. Avoid per-rank small
files, prefer one tarball over 10K loose files; --compress does tar.gz then copies
one file then extracts locally. Coordinate checkpoint writes, do not have every
rank hammer the MDS at once. Keep inputs/outputs on shared FS, hot reads
node-local. Measured on Aurora, staging is extract-bound at 8-64 nodes, about
70-91s flat, and broadcast-bound past 128 nodes, each doubling adding about
1.5-1.8x; a greedy O(log N) fan-out tree keeps it tractable to 4,096 nodes.
</SpeakerNotes>
...
- **20B → 244 GB.** {/* verify from 2026/07/14 auto-retry-native */} Not a file you write casually.
...
EV rule: set the interval where **wasted work on a crash** ≈ **write overhead**.
At a failure every few hours → **tens of minutes**. {/* verify exact cadence from run configs */}

</Col>
</Cols>

Off the critical path → **async checkpointing**, next (and how it took down the cluster).
...
and the LR scheduler, step count, RNG, dataloader position. Our 20B checkpoint is
244 GB, not a file you write casually every step.
...
paying. At a failure every few hours that lands in the tens-of-minutes range. The
write cost is exactly why you want it off the critical path, which is asynchronous
checkpointing.
</SpeakerNotes>
...
**Done right**, async hides the 244 GB write behind the next few hundred steps:

1. **snapshot to host memory** (fast), then hand off
2. **stage, then flush** on a background thread while GPUs train
3. **bound concurrency:** cap how many ranks flush at once

> [!WARNING]+ The naive version took the job down
> At **20B / 512N+**, `--checkpoint.async-mode=async` (244 GB, unbounded) **takes
> the whole job down**: the write contends with training collectives on the same
> fabric. Blunt workaround today: `CHECKPOINT_ASYNC_MODE=disabled`.

A PBS **mid-save kill** once left a stale `step-4500/` **placeholder**. The sync
path assumed it was complete → walltime-blocked for **weeks**. A half-written
...
metadata server and the fabric simultaneously. The point is to overlap the 244 GB
write with the next few hundred steps so cadence costs almost nothing.

The naive version took the job down: at 20B/512N+, turning on
--checkpoint.async-mode=async, that full 244 GB background write unbounded, takes
the whole job down because the write contends with training collectives on the
same fabric and everything stalls. Today's workaround is blunt,
CHECKPOINT_ASYNC_MODE=disabled. And the failure that hides inside checkpointing: a
PBS mid-save kill once left a stale step-4500 placeholder directory; the sync path
saw it, assumed the save was complete, and the run sat walltime-blocked for weeks.
An --auto-retry relaunch finally drove it 4,400 to 5,400 cleanly. A half-written
...
pulling 244 GB off Lustre at once. Same fan-out that stages the venv works on
checkpoints and datasets, any directory or tarball.
