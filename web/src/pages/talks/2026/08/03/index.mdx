...
{/* source: net-new · mermaid reproduction of the paper's parallelism cartoon (figures/parallelism-*.pdf): DP / FSDP / TP / PP / SP-CP */}
<Slide id="parallelism-cartoon" size="sm">

## The same five, in pictures
...
flowchart LR
    subgraph GPU0
      A0["Layer k<br/>half A"]
    end
    subgraph GPU1
      A1["Layer k<br/>half B"]
    end
    A0 <-->|"all-reduce<br/>(every layer)"| A1
...
<SpeakerNotes>
The same five strategies from the previous table, now as pictures (this is the
cartoon from our pretraining paper, redrawn). Data parallel: the model is
replicated on every GPU, the batch is sliced (x0/x1/...), and gradients are
all-reduced and averaged each step. FSDP/ZeRO: parameters, gradients, and
optimizer state are sharded across GPUs and all-gathered just-in-time as each
layer runs forward, trading communication for memory. Tensor parallel: each
layer's matmul is split across GPUs with an all-reduce of activations every layer,
so it is chatty, keep it on-node. Pipeline parallel: the layer stack is split into
stages on different GPUs with a hand-off between stages, and micro-batches are
pipelined to keep every stage busy. Sequence/context parallel: each GPU holds a
slice of the sequence and its KV blocks rotate around a ring (ring-attention), for
very long context. The takeaway is the same as the table: DP is the default,
everything else answers a specific "will not fit" problem.
</SpeakerNotes>

<style>{`
  #parallelism-cartoon .pcart-grid {
    display: grid;
    grid-template-columns: 1fr 1fr 1fr;
    gap: 0.5lh 2ch;
    align-items: start;
  }
  /* SP/CP panel spans the empty 6th cell so the ring has room. */
  #parallelism-cartoon .pcart-wide { grid-column: span 1; }
  #parallelism-cartoon figure { margin: 0; }
  #parallelism-cartoon figcaption {
    font-size: 0.72em; color: var(--foreground2, #838383);
    line-height: 1.25; margin-block-end: 0.2lh;
  }
  #parallelism-cartoon .mermaid svg { width: 100%; max-width: 100%; height: auto; max-height: 13lh; }
  @media (max-width: 768px) {
    #parallelism-cartoon .pcart-grid { grid-template-columns: 1fr; }
  }
`}</style>
