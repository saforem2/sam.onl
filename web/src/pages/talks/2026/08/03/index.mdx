...
- 🐢 serial: **~1 hr** / 2T tokens
- 🐇 BlendCorpus: **~2 min** / 2T tokens (**30x**)
...
maintenance cost is why we moved to TorchTitan (see the fork-tax slide next).
</SpeakerNotes>
...
uv pip install "git+https://github.com/saforem2/ezpz"  # uv: fast, resolves in seconds
...
Still data parallel: shard the replicated state, gather on demand.

<Cols widths="3 3">
<Col>

- **Stage 1:** shard optimizer states (biggest win: ~2x params)
- **Stage 2:** + gradients
- **Stage 3:** + params (= FSDP default)
...
portable across vendors. The loss is going down. Good.

That was the easy part.

- A training run is not a program you start, it is a program you **babysit for
  weeks**.
- At full-machine scale the interesting question stops being "how fast is a
  step?"
- It becomes: **how cheaply do you recover when (not if) something breaks?**

> [!NOTE]+ The rest of this talk
> The good news is the loss curve. The bad news is everything that can interrupt
> it. The restarts are how you survive the bad news.

<SpeakerNotes>
Re-hook line: "Everyone can start a run. The trick at this scale is finishing
one." The emotional core is next: fabric flaps, silent corruption, hangs, and the
machinery we built to survive them.
</SpeakerNotes>

</Slide>

{/* source: reuse · 2026/07/14:218-226 + 2026/06/03:191-197 critical-batch bullet promoted to a slide */}
<Slide id="critical-batch-size" size="sm">

## Critical batch size: the ceiling on data-parallel scaling

The **critical batch size** $B_{\text{crit}}$ is the largest batch where adding
data-parallel workers still buys near-linear speedup.[^cbs-openai]
...
- doubling the global batch roughly **halves the steps** to a target loss
- more nodes means less wall-clock, nearly free

**Above $B_{\text{crit}}$:**

- speedup **saturates**: you burn more tokens/compute for the _same_ loss
- sample efficiency drops; training gets **unstable**[^cbs-zhang][^cbs-zhou]

</Col>
<Col>

**Why HPC folks hit this wall first:**

- INCITE-scale jobs want **≥ 20% of the machine** (~2k nodes on Aurora)
- pure data-parallel at that node count means a **very large global batch**,
  straight past $B_{\text{crit}}$
- the fix is not "more DP": it is TP/PP/EP + a batch you actually _chose_
- $B_{\text{crit}}$ **grows with model and data scale**, but sub-linearly, so it
  is a moving target you **measure**, not assume.[^cbs-zhang][^cbs-zhou]

</Col>
</Cols>

> [!NOTE]+
> The scheduling policy (grab the whole machine) and the training math (stay
> under $B_{\text{crit}}$) pull in opposite directions. Every slide after this is
> about living in that gap.

[^cbs-openai]: [An Empirical Model of Large-Batch Training](https://arxiv.org/abs/1812.06162) (McCandlish et al. 2018).
...
{/* source: reuse · 2026/07/14 large-batch caveats (~964 mano caveat; ~1087 LR-scaled cliff) */}
<Slide id="large-batches" size="sm">

## Training with large batches: the playbook (and where it breaks)
...
- **LR scaling:** raise the LR with batch (linear or $\sqrt{B}$) so each step
  still moves the weights a meaningful amount[^lb-openai]
- **Warmup:** longer warmup as the batch grows, to survive the noisy early steps
  before statistics settle
- **Stability aids:** grad clipping, a large-batch-tuned optimizer (LAMB), and a
  decent $\varepsilon$
...
- Past $B_{\text{crit}}$: **sample efficiency degrades**, you pay more tokens for
  the same loss
- **Linear LR scaling backfires** at the extreme: at 80B, LR-scaling the 16x
  batch moved the NaN **earlier** (step 7 vs step 29)
- **LRs do not transfer across batch:** `mano` tuned at GBS=48 loses to AdamW at
  GBS=384 (**2.88 vs 2.71**) until re-tuned

</Col>
</Cols>

- Practical rule: **re-tune at the target batch and node count**, do not
  extrapolate a small-batch LR up a 100x jump and hope.

[^lb-openai]: [An Empirical Model of Large-Batch Training](https://arxiv.org/abs/1812.06162) (McCandlish et al. 2018): LR and warmup both need to grow with batch, and the payoff flattens at the critical batch size.
...
AdamW is the baseline. Everything else is buying **curvature** (a better
preconditioner) at some compute price.

<div class="tight-table">

| Optimizer | Family | Core idea |
| --- | --- | --- |
| **AdamW**[^ol-adamw] | first-order baseline | per-coordinate (diagonal) 2nd moment + decoupled weight decay |
| **LAMB**[^ol-lamb] | large-batch 1st-order | Adam + layer-wise **trust ratio** on the LR (unlocks huge batches) |
| **Adopt**[^ol-adopt] | fixed-Adam | drops the current grad from the 2nd moment, converges for **any** $\beta_2$ |
| **Sophia**[^ol-sophia] | diagonal 2nd-order | cheap **Hessian-diagonal** preconditioner, clipped; light curvature |
| **Shampoo**[^ol-shampoo] | structured 2nd-order | **Kronecker-factored** preconditioner per layer (factors of $GG^\top$) |
| **SOAP**[^ol-soap] | Shampoo + Adam | run Adam **in Shampoo's eigenbasis**: stabler, fewer knobs |
| **Muon**[^ol-muon] | spectral / precond. | **Newton-Schulz orthogonalize** the momentum: a cheap Shampoo-like step |

</div>

- The bottom three (**Shampoo, SOAP, Muon**) are one family: precondition the
  update with second-moment structure, differing only in **how cheaply** they
  approximate it.

[^ol-adamw]: [Decoupled Weight Decay Regularization](https://arxiv.org/abs/1711.05101) (Loshchilov & Hutter 2019).

[^ol-lamb]: [Large Batch Optimization for Deep Learning: Training BERT in 76 minutes](https://arxiv.org/abs/1904.00962) (You et al. 2020).

[^ol-adopt]: [ADOPT: Modified Adam Can Converge with Any $\beta_2$ with the Optimal Rate](https://arxiv.org/abs/2411.02853) (Taniguchi et al. 2024).

[^ol-sophia]: [Sophia: A Scalable Stochastic Second-order Optimizer](https://arxiv.org/abs/2305.14342) (Liu et al. 2023).

[^ol-shampoo]: [Shampoo: Preconditioned Stochastic Tensor Optimization](https://arxiv.org/abs/1802.09568) (Gupta et al. 2018).

[^ol-soap]: [SOAP: Improving and Stabilizing Shampoo using Adam](https://arxiv.org/abs/2409.11321) (Vyas et al. 2024).

[^ol-muon]: [Muon: MomentUm Orthogonalized by Newton-Schulz](https://arxiv.org/abs/2502.16982) (Jordan et al. 2024/2025).

<style>{`
  #optimizer-landscape table { table-layout: auto; width: max-content; max-width: 100%; font-size: 0.9em; }
  #optimizer-landscape table th, #optimizer-landscape table td { white-space: nowrap; }
  #optimizer-landscape table th:nth-child(3), #optimizer-landscape table td:nth-child(3) { white-space: normal; }
`}</style>

</Slide>

{/* source: net-new · Vyas et al. 2024 (arXiv:2409.11321); authors' speedups kept off-slide */}
<Slide id="soap" size="sm">

## SOAP: Adam in Shampoo's eigenbasis

**Shampoo** is preconditioned gradient descent: for each weight matrix it keeps a
**Kronecker-factored** second-moment estimate (left factor from $GG^\top$, right
factor from $G^\top G$) and steps along the preconditioned gradient. Powerful,
but the eigendecompositions and extra knobs (grafting, $\varepsilon$,
preconditioning power/frequency) make it **expensive and fiddly** at scale.

<Cols widths="1 1">
<Col>

**The SOAP idea**[^soap-paper]

- Shampoo's preconditioner defines a **rotated coordinate frame** (its
  eigenbasis)
- **Run plain Adam inside that frame:** cheap per-coordinate 2nd moments, done in
  the good basis
- Refresh the eigenbasis only **every $k$ steps** (it drifts slowly), so the
  costly eigendecomp is **amortized**
...
- **Fewer hyperparameters** than Shampoo: essentially AdamW + one knob (eigenbasis
  update frequency)
- **More stable:** Adam's normalization tames Shampoo's rough edges
- **Keeps the win:** the preconditioning benefit survives, at close to Adam's
  per-step cost

</Col>
</Cols>

- **Relation to Muon:** Muon is the **cheaper cousin** in the same family. It
  approximates the same spectral/preconditioned update with **Newton-Schulz
  orthogonalization** instead of an explicit eigendecomposition, so it skips
  SOAP's basis machinery entirely.[^soap-muon]

[^soap-paper]: [SOAP: Improving and Stabilizing Shampoo using Adam](https://arxiv.org/abs/2409.11321) (Vyas, Morwani, Zhao, et al. 2024). The paper shows Shampoo (1/2 power) is equivalent to running Adafactor in Shampoo's eigenbasis; SOAP generalizes this by running full Adam there.

[^soap-muon]: [Muon](https://arxiv.org/abs/2502.16982) (Jordan et al. 2024/2025): orthogonalized-momentum update as a low-overhead spectral approximation.

<SpeakerNotes>
One-sentence version: Shampoo tells you a good basis, then you do the boring
cheap thing (Adam) in it. The equivalence in the paper is the hook: 1/2-power
Shampoo is Adafactor-in-the-eigenbasis, so SOAP is the natural generalization
(Adam, not Adafactor, in that basis). Cost story: the eigendecomposition is the
expensive part, and you reuse the basis for many steps because it barely moves,
so amortized overhead over AdamW is small. Their reported numbers (LLM
pre-training vs AdamW) are roughly 40% fewer steps / 35% less wall-clock; I keep
those off the slide since they are the authors' runs, not ours. Where it fits our
story: the principled-but-heavier end of the family. mano (next) chases the same
quality-per-flop from the opposite direction, manifold normalization instead of
eigenbases.
</SpeakerNotes>
...
- **`mano` (our 2B speed win) died first** (step 5), despite the _safest_-looking
  LR band. **Early-step ranking does not predict sustained stability.**
- Shared failure across 3 optimizers means a **corner-level instability (bf16 at
  dim=9216)**, not tuning. Grad-path triggers: `LBS>1`, large `dp_degree`.
- **Fix in flight:** long warmup (≥200 steps) + grad clipping, possibly an fp32
  grad path. Stable corner today: **TP=4 · LBS=1 · GBS=372**, 100/100 steps
  NaN-free (loss 12.93 to 7.72) at ~9.8% MFU.
...
<figure>
    ...
    <figcaption>
        80B LR-finder at the production batch (GBS=6,144). **AdamW and muon cliff
        straight to NaN** with no usable minimum (× marks the diverged LRs);
        `mano` and SophiaG reach a real minimum first (circled, ~11.9 / ~11.8)
        but SophiaG blows up just past it.
    </figcaption>
</figure>
...
- Say each node is healthy: it fails on average **once a year**.
- Put 16,000 of them in one job: that is **~44 failures a day**, purely from
  expected value.
- The math is unforgiving, and everyone who has done it confirms it:

| Run | Scale | Failures |
| --- | --- | --- |
| **Llama 3 405B**[^llama3] | 16K H100s · 54 days | **419** failures (≈ **1 every 3h**); 99% auto-recovered |
| **OPT-175B**[^opt] | ~1K A100s · 2 mo | 35 manual restarts + 100+ cycled hosts |
| **BLOOM-176B**[^bloom] | 384 A100s · 3.5 mo | frequent loss spikes; embedding-norm + checkpoint cadence |
| **GLM-130B**[^glm] | | spikes "increasingly frequent"; some recover, some go NaN |
...
**transient** (retry and move on) or **systemic** (stop, or you corrupt the whole
run).
...
- **Hardware:** ECC errors, dead XPU, thermal throttle, a bad node in your
  allocation
- **Software:** CheckpointError from routing non-determinism, OOM, a bad upstream
  commit
- **Network:** fabric flaps, gloo `Connection closed by peer`, collective hangs
- **System:** Lustre stalls, PBS walltime, the scheduler handing you a node it
  should have drained
...
- **Transient:** one bad node, a flaked collective, a mid-save kill. Retry on
  fresh hardware and you are fine.
- **Systemic:** a config bug, a corrupted shard, a silent numerical bug. Retrying
  just reproduces it faster.

**The whole operational cost is telling these two apart.** Retry a systemic fault
and you burn the spare pool. Halt on a transient one and you waste walltime you
already paid for.

</Col>
</Cols>

The nasty ones are the **silent** faults: no traceback, no crash, the loss curve
still looks fine. Two of those coming up.
...
switch hiccups, and one collective somewhere stops making progress. How it shows
up depends on the backend:

- **Loud version:** `gloo` TCP `Connection closed by peer` a few hours in (job
  `8470102`, 20B/256N). At least it crashed, so you get an exit code to act on.
- **Quiet version:** a collective just **stops**. Every rank blocks in the same
  `all_reduce`, waiting on a peer that will never answer.

The quiet version is the dangerous one:

- The process is **alive** by `kill -0`. Nothing has crashed.
- On XPU, `xccl` silently ignores `train_timeout_seconds`, so the hung collective
  sits there consuming your **entire PBS walltime**.
- We watched job `8479579` (20B/512N) hang at **step 803** with the heartbeat
  still ticking. Alive, and doing nothing.

**The one signal you can trust:** a collective hang **quiets stdout**. Every rank
is blocked in the same call, so nothing reaches the log. Absence of progress is
the detector (see the watchdog, later).
...
**Symptom.** Loss curves looked reasonable. But lm-eval scores never moved:
ARC-Easy stuck at **~0.27** (random baseline) for **17,000+ steps**.

**Cause.** `training.dtype=bfloat16` gave us a bf16 master copy. RMSNorm weights
init at `1.0`, and bf16 ULP at scale 1.0 is `~7.8e-3`. The per-step optimizer
update is `~1.6e-5`, so **every update rounds to zero**. All 25 RMSNorm tensors
sat at exactly `1.0` from step 100 to step 17,400.

**Why everything else trained fine.** Linear layers init at `std≈0.02`, where
bf16 ULP is `~3.8e-5`: same magnitude as the update, so it registers. RMSNorm's
larger init scale means coarser ULP, and the update vanishes in rounding.

**Fix.** Default `training.dtype=float32`, FSDP
`MixedPrecisionPolicy(param_dtype=bf16, reduce_dtype=fp32)`. Master is fp32,
forward/backward stay bf16. Costs ~1 GB extra master at 2B, ~10 GB at 20B, both
under budget.
...
> [!WARNING]+ The lesson
> Loss looks like training. lm-eval is the only ground truth for "is the model
> actually learning?" Add a periodic eval gate, or you can burn 17K steps
> teaching a model nothing.
...
**Symptom.** Step-1 loss for `agpt_*` (vocab = 256,128) should be
`ln(256128) ≈ 12.45`.

- On **TP=1** we saw `12.95` ✓
- On **TP=2**, after upstream commit `1786292d` (2026-04-27): step-1 reported as
  **`1.07`** ✗
- That is exactly `12.84 / 12`, where `dp_world_size = 12`. The loss was being
  divided by the wrong world size.

**Cause.** `_dist_reduce()` short-circuits on DTensor via `full_tensor()`. But the
loss is **Replicated on the TP mesh**, and the reduction was requested over
`batch_mesh` (orthogonal). The short-circuit silently drops the cross-batch sum.

**Why it survived review.** Gradients and optimizer steps are **correct**. Only
the `loss:` field that lands in stdout and W&B is wrong: the curves look
reasonable, just `1/12` of the true value.

- Filed as `pytorch/torchtitan#3204`; our workaround calls `loss.full_tensor()`
  before `dist_sum` in `ezpz/trainer.py:503-516`.

**Lesson.** A **bit-exact smoke gate** caught this immediately. The prior 2B TP=1
baseline gave us a number to disagree with. Without a reference, a wrong-but-plausible
loss is invisible.
...
Lustre is shared by the whole facility, and your job is one of many tenants.
Every file operation is a small tax, and at scale a small tax paid by every rank
turns into minutes of dead time before training even starts.
...
- **Metadata pressure.** `rsync` `stat()`s every file; a venv is tens of
  thousands of tiny files. The first copy is slow not because of bytes but
  because of **metadata round-trips**.
- **Contention.** You share bandwidth and metadata servers with everyone else on
  the machine. Your "fast" copy is at the mercy of their job.
- **Fan-out limits.** A single source saturates its NIC at ~8 targets, so naive
  broadcast to thousands of nodes is `O(N)`.
...
- **Stage to node-local `/tmp`.** `ezpz yeet` copies the venv (or dataset, or
  checkpoint) to `/tmp` on every node once, then imports hit local SSD instead of
  Lustre.
- **Avoid per-rank small files.** Prefer one tarball over 10K loose files:
  `--compress` does `tar.gz` then copy one file then extract locally.
- **Coordinate checkpoint writes.** Do not have every rank hammer the MDS at once
  (more on async next).
- Keep **inputs/outputs** on shared FS; keep **hot reads** node-local.

</Col>
</Cols>

Measured on Aurora: staging is **extract-bound at 8-64 nodes** (~70-91s flat) and
**broadcast-bound past 128 nodes** (each 2x in node count adds ~1.5-1.8x
wall-clock). A greedy `O(log N)` fan-out tree, not a star, is what keeps it
tractable to 4,096 nodes.

</Slide>

{/* source: refresh · 20B ckpt = 244 GB (2026/07/14:1599); cadence marked verify */}
...
last one, so everything since then is wasted work.
...
- Optimizer state (the big one: Adam-family is ~2x the model)
- LR scheduler, step count, RNG, dataloader position

**How big**

- Our **20B** checkpoint is **244 GB**. {/* verify from 2026/07/14 auto-retry-native */}
- That is not a file you write casually every step.
...
- **Too frequent:** checkpoint write overhead eats throughput.
- **Too rare:** a crash throws away hours of compute.

Expected-value rule of thumb: pick an interval where the **expected wasted work
on a crash** (about half an interval, times your failure rate) is comparable to
the **write overhead** you are paying. At a failure every few hours, that lands
you in the tens-of-minutes range. {/* verify exact cadence from run configs */}

</Col>
</Cols>

The write cost is exactly why you want it **off the critical path**. That is
asynchronous checkpointing, next, along with the way it once took down the whole
cluster.
...
**Done right**, async checkpointing hides the write behind compute:

1. **Snapshot to host memory** on the training thread (fast), then hand off.
2. **Stage, then flush.** A background thread writes staged bytes to the
   filesystem while the GPUs keep training.
3. **Bound concurrency.** Cap how many ranks flush at once so you do not stampede
   the metadata server and the fabric simultaneously.

The point is to overlap the 244 GB write with the next few hundred steps, so
cadence costs you almost nothing.

> [!WARNING]+ The naive version took the job down
> At **20B / 512N+**, turning on `--checkpoint.async-mode=async` (that full **244
> GB** background write, unbounded) **takes the whole job down**. The write
> contends with training collectives on the same fabric and everything stalls.
> Today's workaround is blunt: `CHECKPOINT_ASYNC_MODE=disabled`.

And the failure that hides inside checkpointing: a PBS **mid-save kill** once left
a stale `step-4500/` **placeholder** directory. The sync path saw it, assumed the
save was complete, and the run sat walltime-blocked for **weeks**. An
`--auto-retry` relaunch (next) finally drove it **4,400 to 5,400** cleanly. A
...
clock runs from crash to first-step-back-training, not from crash to "file
exists."

**Reload fast**

- **Stage the checkpoint node-local.** `ezpz yeet ~/models/checkpoint-729` pushes
  it to `/tmp` on every node, so the reload hits local SSD instead of every rank
  pulling 244 GB off Lustre at once.
- Same fan-out that stages the venv works on checkpoints and datasets: any
  directory or tarball.

**Reload anywhere: checkpoint converters**

- **Megatron ⇄ 🤗 HF ⇄ ZeRO ⇄ Universal.** The training-format checkpoint and the
  format you evaluate, serve, or fine-tune from are not the same thing.
- **Universal** decouples the checkpoint from the parallelism layout, so you can
  resume at a **different** TP/PP/DP config than you saved at (recover on whatever
  the scheduler gives you, not only on an identical allocation).

**It pays off downstream too:** `checkpoint-729-hf` is now a first-class
production asset. It is the base our SFT and GRPO stages build on, so "fast,
portable reload" is not just a restart nicety, it is how the model leaves
pre-training.
...
This moved **out of bash and into `ezpz`**:
...
- Classifies each attempt's exit: `success` / `walltime` / `bad-node` /
  `stuck-pre-training`.
- On **bad-node**: scrapes the failing host from the log, swaps in a spare,
  re-execs.
- **Guards against config bugs**: 2 consecutive attempts with zero `step=`
  markers, and it stops. Do not burn the whole spare pool on a broken run.

**It broke the 20B/512N stall.** That stale `step-4500/` placeholder from a PBS
mid-save kill had the sync chain walltime-blocked for **weeks**. An `--auto-retry`
relaunch drove it **4,400 to 5,400** cleanly.

Ships in [`saforem2/ezpz#144`](https://github.com/saforem2/ezpz/pull/144). Same
scraper as the bash-lib path, a pure-Python loop on top.
...
The PROCESS layer, and the one that catches what nothing else does.

**The problem.** `xccl` on XPU silently ignores `train_timeout_seconds`, so a job
stuck in a hung collective sits consuming the full PBS walltime instead of
aborting. But every collective hang **quiets stdout** (every rank blocks in the
same call, nothing reaches the log). That silence is the signal we act on.
...
- **`--timeout SECONDS`**: kill the launched process if its stdout goes **idle**
  (not walltime) for this many consecutive seconds. Returns exit code 124
  (matches GNU `timeout(1)`).
- **`--retries N`**: re-execute on any non-zero exit (including the watchdog's
  124) up to N times. Exponential backoff: 5s, 10s, 20s, 40s, 60s (capped).
...
The watchdog fires on the **absence of progress**, not on a heartbeat ping. A
hung job is "alive" by `kill -0`, which is exactly why a heartbeat can lie to you.

**Scope caveat.** It watches only the process `ezpz launch` spawns directly. If
`qsub` runs a wrapper that internally invokes `python train.py`, the watchdog
needs to live inside that script (or wrap the inner call with `ezpz launch` too).
...
None of this is theory. **Job `8505298`, 2026-05-23.** Attempt 1 trains cleanly
steps 1 to 37, then the log goes **completely silent** at step 37. No traceback,
no MPI error, no rank dying. Just dead.
...
production-readiness:** the `--timeout=1800` watchdog, the `exit 124`
classification (distinct from PBS `exit 143`), and `failover_swap_one_blind()`
for when no specific bad node can be identified. They did. The run kept going,
unattended, at 9:36 PM on a Friday.

Full writeup:
[`20260523-failover-silent-hang-recovery-8505298.md`](https://github.com/saforem2/torchtitan/blob/ezpz/torchtitan/experiments/ezpz/docs/experiments/agpt/aurora/20260523-failover-silent-hang-recovery-8505298.md)
