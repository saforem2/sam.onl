...
<style>{`
  ...
  @media (max-width: 768px) {
      /* Let the diagram fill the slide width AND grow much taller
         so it claims real estate on portrait screens. Mermaid renders
         the russian-doll at a wide intrinsic aspect; without a min
         height the auto-scaled SVG sits in a thin band at the top.
         Force min-height: 60vh so it visibly fills the slide; the
         aspect-ratio: auto + width: 100% combo will stretch the SVG
         vertically (the nested boxes get taller, text size stays
         readable because we already bumped font-size: 1.25em). */
      #restart-cascade .mermaid {
          min-height: 60vh;
          display: flex;
          align-items: stretch;
      }
