...
flowchart TB
    subgraph job["📋 JOB scope — PBS · hours"]
        JOB_TXT["<i>crash / walltime</i> →<br/><b>chained resubmit</b>"]
        subgraph node["🖥 NODE scope — failover wrapper · minutes"]
            NODE_TXT["<i>bad host detected</i> →<br/><b>swap from spare pool</b>"]
            subgraph proc["⏱ PROCESS scope — ezpz launch · seconds"]
                PROC_TXT["<i>stdout idle ≥ N s</i> →<br/><b>kill + backoff</b>"]
            end
        end
    end
