...
**Failover engineering deep-dive** — the production table + qsub
wrapper, plus the four-piece system that swaps spares in on crash:

- `ezpz launch --timeout / --retries` — stdout-idle watchdog
- watchdog state machine — one hang/recover cycle
- `--auto-retry` — bad-node failover, on tap
- in-production catch — silent hang at step 37, recovered cleanly

**Cross-version churn** — APIs shift faster than vendor support
catches up. Example: `DeviceMesh` + AC + TP=2 + torch 2.13 →
AOT-autograd assertion on every 80B config; pin torch 2.10 until
...
> a nightly PyTorch release — the stdout-idle watchdog won't stay
> load-bearing.
...
<figure>
    <img
        src="/talks/2026-06-03/figures/optimizers-r5-loss-curves.webp"
