...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-06-03/figures/eval-2b-compare-mobile.svg"
        />
        <img
...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-06-03/figures/eval-20b-compare-mobile.svg"
        />
        <img
...
flowchart TB
    subgraph proc["⏱ PROCESS · seconds"]
        PROC_TXT["ezpz launch watchdog<br/>stdout idle ≥ N s<br/>→ kill + backoff"]
    end
    subgraph node["🖥 NODE · minutes"]
        NODE_TXT["failover wrapper<br/>bad host detected<br/>→ swap from spare pool"]
    end
    subgraph job["📋 JOB · hours"]
        JOB_TXT["PBS chained resubmit<br/>crash / walltime<br/>→ next job in chain"]
    end
...
<style>{`
  ...
  /* Desktop/mobile variant swap. Default: show desktop nested-russian-doll
     diagram; hide the tall mobile variant. On narrow viewports flip both
     so the tall sibling-stack version fills the portrait band instead.
     Both variants reuse the #restart-cascade-scoped chip/badge/shadow
     rules above — the wrapper divs only gate visibility, the inner
     .mermaid block inherits all styling identically. */
  ...
  @media (max-width: 768px) {
      ...
      #restart-cascade .cascade-mobile .mermaid svg {
          width: 100%;
          max-width: 100%;
          height: auto;
      }
