...
1. **The Good**
   - The stack (`ezpz` · `torchtitan-fork` · `blendcorpus`)
...
- 🧠 [`saforem2/torchtitan@ezpz`](https://github.com/saforem2/torchtitan/tree/ezpz): FSDP · TP · PP · EP · MoE · AC + XPU/SYCL patches
...
**The claim:** same code, every vendor — no `if device == "xpu"` branches in user code.
...
| Spec         | Value                                                                  |
| ------------ | ---------------------------------------------------------------------- |
| Architecture | 1.986B params, 12 layers, GQA (16h / 4 kv), 8192 ctx[^batch-tokens], LBS=2 |
...
<figure>
    ...
    />
    <figcaption>
        Same MDS trajectory as the reference chart, with the
        `torchtitan` 256N production run drawn on the same axes —
        starts inside the MDS pretrain region and tracks the same
        loss-vs-tokens curve.
    </figcaption>
</figure>
...
<figure>
    ...
    />
    <figcaption>
        256N / GBS=6,144. At matched tokens, **δ ≈ 0.02** — within
        run-to-run noise. The cutover preserved training behavior.
    </figcaption>
</figure>
...
<figure>
    ...
    />
    <figcaption>
        Data: [`docs/evals/agpt/2b`](https://github.com/saforem2/torchtitan/tree/ezpz/torchtitan/experiments/ezpz/docs/evals/agpt/2b),
        production run: [`docs/production/agpt/2b`](https://github.com/saforem2/torchtitan/tree/ezpz/torchtitan/experiments/ezpz/docs/production/agpt/2b)
    </figcaption>
</figure>
...
<figure>
    <img
        src="/talks/2026-06-03/figures/eval-20b-compare.svg"
        alt="All-production eval overlay: 2B MDS, 2B TT 256N, 2B TT 512N, 20B TT 512N"
    />
    <figcaption>
        Data: [`docs/evals/agpt/20b`](https://github.com/saforem2/torchtitan/tree/ezpz/torchtitan/experiments/ezpz/docs/evals/agpt/20b),
        production run: [`docs/production/agpt/20b`](https://github.com/saforem2/torchtitan/tree/ezpz/torchtitan/experiments/ezpz/docs/production/agpt/20b)
    </figcaption>
</figure>
...
Three patterns we hit over and over running on non-NVIDIA:

- **Silent fallbacks.** Vendor-specific fast paths get dropped inside abstractions (FSDP, `torch.compile`, AC) with no warning. _e.g._ XPU `OVERRIDEABLE` SDPA kernel is **23×** faster than `MATH` — silently swapped out under FSDP.
- **NVIDIA-shaped defaults.** "Sensible" microbatch / memory / collective settings target H100; on Intel Max they OOM, on AMD they hang. Every default needs an XPU-aware override path.
- **Cross-version churn.** Upstream APIs shift faster than vendor support catches up. `DeviceMesh` + AC + TP=2 + torch 2.13 → AOT-autograd assertion on every 80B config; pin torch 2.10 until it lands.

**The meta-lesson:** at scale, the deepest portability bugs aren't "wrong answer" — they're _silently slower_ or _silently more memory_, and you only catch them with bit-exact regression gates.
...
flowchart TB
    subgraph job["📋 JOB scope — PBS · hours"]
        JOB_TXT["crash / walltime → chained resubmit"]
        subgraph node["🖥 NODE scope — failover wrapper · minutes"]
            NODE_TXT["bad host detected → swap from spare pool"]
            subgraph proc["⏱ PROCESS scope — ezpz launch · seconds"]
                PROC_TXT["stdout idle ≥ N s → kill + backoff"]
            end
        end
    end
...
<figure>
    ...
    />
    <figcaption>
        Cross-optimizer sweep on Aurora (Intel Max 1550, 2 nodes / 24
        XPUs). Source:
...
<figure>
    ...
    />
    <figcaption>Optimal LR per config. AdamW most tolerant; SophiaG ~10× lower.</figcaption>
</figure>
...
<figure>
    ...
    />
    <figcaption>
        Round 5 speedrun: 8 nodes, GBS=384, 10B tokens. Dual view —
        by step and by tokens consumed.
    </figcaption>
</figure>
...
{/*───────────────────────────────────────────────────────────────
Appendix: yeet-env tarball broadcast scaling
───────────────────────────────────────────────────────────────*/}

<Slide id="yeet-env">

## `yeet-env` — venv tarball broadcast at scale

<div class="yeet-env-grid">

<figure>
    <img
        src="/talks/2026-06-03/figures/yeet-env-seconds.png"
        alt="Total wall-clock for yeet-env broadcast vs node count, 8 to 4096 nodes"
    />
    <figcaption>Total wall-clock (8 → 4096 nodes).</figcaption>
</figure>

<figure>
    <img
        src="/talks/2026-06-03/figures/yeet-env-per-node.png"
        alt="Per-node amortized cost vs node count, 8 to 4096 nodes"
    />
    <figcaption>Per-node amortized cost. 48× efficiency over the sweep.</figcaption>
</figure>

</div>

**Two regimes.** 8-64N is extract-bound (~70-90s flat, fixed-cost
extraction amortizes); ≥128N is broadcast-bound, total grows
super-linearly. At 4096 nodes the full Aurora broadcast finishes in
~13 min. The pre-tarball per-file rsync mode predicted hours at
256+ nodes — never reached production.
...
[^batch-tokens]: LBS=2 × 3,072 GPUs × 8192 ctx ≈ **50M tokens/step**
