...
<figure>
    ...
    <figcaption>
        256N · GBS=6,144 (50M tok/batch · 8192 ctx · LBS=2), aggregated
        across every restart chain per optimizer. **SophiaG** is the
        only one that keeps descending to loss ≈ 2.4 with bounded grad
        norms; Lamb plateaus ~3.0, MuonClip drifts up + spikes, Muon
        diverges before 250B tokens, AdamW dies in the warmup. That's
        why SophiaG is the optimizer in every 2B/20B reference chart
        that follows.
    </figcaption>
</figure>
