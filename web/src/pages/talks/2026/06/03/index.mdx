...
9. **Silent-hang detection** — `ezpz launch --timeout / --retries`
10. **What generalizes — what doesn't**
11. **The ask** — open questions for TPC
...
**The problem.** `xccl` on XPU silently ignores `train_timeout_seconds`,
so a torchtitan job stuck in a hung collective sits consuming the full
PBS walltime instead of aborting. Every collective hang quiets stdout
(every rank blocks in the same call, nothing reaches the log) — that's
the signal we can act on.
...
- **`--timeout SECONDS`** — kill the launched process if its stdout
  ...
  `SIGTERM`, 10s grace, then `SIGKILL`. Returns exit code 124
  (matches GNU `timeout(1)`) so wrappers can tell "killed for silence"
  from "command failed".
...
(or you wrap the inner call with `ezpz launch` too).

> **Open issue we'd like to push upstream:** make `xccl` honor
> `train_timeout_seconds` (or the equivalent NCCL knob) on XPU so
> the watchdog isn't load-bearing.
