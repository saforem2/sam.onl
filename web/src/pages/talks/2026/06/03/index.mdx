...
_Backups_: optimizer experiments (Mano · Muon · SophiaG · LR-finder), silent-correctness bugs (bf16 RMSNorm freeze, TP loss reporting), `blendcorpus` deep-dive
...
<figure>
    ...
    />
    <figcaption>Both v2 chains track MDS at matched tokens.</figcaption>
</figure>
...
```mermaid
flowchart LR
    subgraph proc["⏱ process — ezpz launch"]
        direction TB
        RUN["python train.py"] -. "idle ≥ N s" .-> TERM["kill + backoff"]
        TERM --> RUN
    end
    subgraph node["🖥 node — failover wrapper"]
        direction TB
        ACTIVE["active pool"] -. "bad node" .-> SWAP["swap in spare"]
        SWAP --> ACTIVE
    end
    subgraph job["📋 job — PBS"]
        direction TB
        A1["attempt N"] -. "crash / walltime" .-> A2["attempt N+1"]
        A2 --> A1
    end

    proc -. "spare pool drained" .-> node
    node -. "out of spares" .-> job
    ...
    class RUN,TERM procC
    class ACTIVE,SWAP nodeC
    class A1,A2 jobC
```

Inner loops handle most failures; outer loops handle the rest. Seconds → minutes → hours.
