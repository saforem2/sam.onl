...
title: 'Pre-Training AuroraGPT at Scale on Aurora'
...
<a
    ...
    href="https://samf.sh/talks/2026/07/14"
    aria-label="QR code linking to these slides">
    <img
        src="/talks/2026-07-14/figures/qr-slides.svg"
        alt="QR code linking to samf.sh/talks/2026/07/14"
    />
    <span>samf.sh/talks/2026/07/14</span>
</a>

# <span style="color:var(--foreground0);">Pre-Training AuroraGPT at Scale on Aurora</span>

<p class="title-slide-thesis">
    Less a story of peak FLOP/s than of what works, what breaks, and how cheaply
    you recover.
</p>
...
<p class="author-list">
    <strong>Sam&nbsp;Foreman</strong>[^anl], Nathan&nbsp;Nichols,
    Varuni&nbsp;Sastry, Samuel&nbsp;Wheeler, Khalid&nbsp;Hossain,
    Huihuo&nbsp;Zheng, Murali&nbsp;Emani, Filippo&nbsp;Simini,
    Marieme&nbsp;Ngom, Ethan&nbsp;Wong, Venkat&nbsp;Vishwanath
</p>
...
2. **The Bad**:
    ...
    - At scale, _failure is the default_
...
[^mds]: And _away_ from [argonne-lcf/Megatron-DeepSpeed](https://github.com/argonne-lcf/Megatron-DeepSpeed)!
...
<figure>
    ...
    <figcaption>
        **SophiaG** is the only one that stays in the low-loss band with bounded
        grad norms.
    </figcaption>
</figure>
...
<figure>
    <picture>
        ...
        />
        <img
            src="/talks/2026-06-03/figures/lr-finder-aurora.svg"
...
<div class="why-titan-grid">

<div class="why-titan-left">
...
<style>{`
  ...
  #why-titan .why-titan-left { min-width: 0; }
  #why-titan .why-titan-right { min-width: 0; }
  ...
  @media (max-width: 768px) {
    #why-titan .why-titan-grid { grid-template-columns: 1fr; gap: 1lh; }
  }
`}</style>
...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-07-14/figures/eval-combined-4bench-mobile.svg"
        />
        <img
            src="/talks/2026-07-14/figures/eval-combined-4bench.svg"
...
<style>{`
  ...
  /* Recolor + thicken the connectors so the edges read at slide
     distance next to the bumped-up (1.6em) node text. The arrowhead
     markers themselves are SVG geometry (markerWidth/Height attrs,
     not CSS), so they're enlarged once per render in mermaid-render.ts
     (see the #fork-tax marker-scaling block there). */
  ...
  /* Edge labels ("retry", the ✅/❌ badges) default to 16px — tiny next
     to the 1.6em node text. Bump the leaf text and let it overflow
     mermaid's fixed 40x20 foreignObject box. */
  #fork-tax .mermaid .edgeLabel foreignObject { overflow: visible; }
  #fork-tax .mermaid .edgeLabel span,
  #fork-tax .mermaid .edgeLabel p {
    font-size: 1.5em;
  }
`}</style>
...
{/*───────────────────────────────────────────────────────────────
Optimizers: mano (2B speed win) + the 80B cliff (moved into main flow)
───────────────────────────────────────────────────────────────*/}
...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-07-14/figures/optimizer-loss-mobile.svg"
        />
        <img
            src="/talks/2026-07-14/figures/optimizer-loss.svg"
...
{/*───────────────────────────────────────────────────────────────
Appendix: 80B optimizer cliff (one bf16 corner, three dead optimizers)
───────────────────────────────────────────────────────────────*/}
...
<style>{`
  /* Pin the two short columns so Signature gets the room and stops
     wrapping (fixed-layout table otherwise splits width evenly). The
     first two columns nowrap so "Optimizer" / "Died at" / "step 14"
     can't break mid-cell; widths include cell padding. */
  #optimizer-cliff table { table-layout: fixed; width: max-content; max-width: 100%; }
  #optimizer-cliff table th:nth-child(1), #optimizer-cliff table td:nth-child(1) { width: 13ch; white-space: nowrap; } /* Optimizer */
  #optimizer-cliff table th:nth-child(2), #optimizer-cliff table td:nth-child(2) { width: 11ch; white-space: nowrap; } /* Died at */
  #optimizer-cliff table th:nth-child(3), #optimizer-cliff table td:nth-child(3) { width: 34ch; } /* Signature */
`}</style>

- **`mano` (our 2B speed win) died first** (step 5), despite its 80B
  LR-finder band looking the _safest_ of the three. **Early-step ranking
...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-07-14/figures/lr-finder-80b-mobile.svg"
        />
        <img
            src="/talks/2026-07-14/figures/lr-finder-80b.svg"
...
<style>{`
  #optimizer-cliff-detail .cliff-grid {
    display: grid;
  ...
  #optimizer-cliff-detail .cliff-left { min-width: 0; }
  #optimizer-cliff-detail .cliff-right {
    min-width: 0;
    display: flex;
    flex-direction: column;
    justify-content: center;
  }
  #optimizer-cliff-detail .cliff-right figure { margin: 0; width: 100%; }
  #optimizer-cliff-detail .cliff-right img { width: 100%; height: auto; }
  @media (max-width: 768px) {
    #optimizer-cliff-detail .cliff-grid { grid-template-columns: 1fr; gap: 1lh; }
  }
...
<style>{`
  /* LR on wide, TB on narrow: two mermaid blocks, one hidden per
     breakpoint. mermaid-render.ts skips 0x0 (display:none) blocks and
     re-renders the visible one when the 768px query flips. */
  #posttrain-intro .pt-mobile { display: none; }
  @media (max-width: 768px) {
    #posttrain-intro .pt-desktop { display: none; }
    #posttrain-intro .pt-mobile { display: block; }
  }
...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-07-14/figures/cpt-loss-mobile.svg"
        />
        <img
            src="/talks/2026-07-14/figures/cpt-loss.svg"
...
<div class="repo-card repo-card-warning">
    ...
    (−10.4pp) vs olmo50-dolmino50 <strong>0.610</strong> (holds). HellaSwag fell
    for both. <strong>No mixing ratio declared a winner</strong>. Hypothesis:
    re-warm to peak LR caused forgetting; gentle-LR retry pending.
</div>
...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-07-14/figures/sft-curves-mobile.svg"
        />
        <img
            src="/talks/2026-07-14/figures/sft-curves.svg"
            alt="AuroraGPT-2B SFT loss curves on tulu_math_uc_mix"
        />
    </picture>
    <figcaption>
        Run 1 **complete: final loss 0.77** over 4.5B tokens (3 epochs, 32N,
        GBS=6,144).
    </figcaption>
</figure>
...
<figure>
    <picture>
        <source
            media="(max-width: 768px)"
            srcset="/talks/2026-07-14/figures/grpo-curves-mobile.svg"
        />
        <img
            src="/talks/2026-07-14/figures/grpo-curves.svg"
...
{/*───────────────────────────────────────────────────────────────
What it took to train AERIS (same stack / machine)
───────────────────────────────────────────────────────────────*/}

<Slide id="aeris" size="sm">

## What it took to train AERIS

On the **same machine** (Aurora), but with its **own separate stack**,
**AERIS**[^aeris] is a pixel-level Swin **diffusion transformer** for
generative weather & climate prediction, the first diffusion model taken
to this scale, a useful contrast point for what full-machine training
demands.
...
- A separate codebase, but the **same operational realities** we hit:
  bf16 stability, full-machine scaling, and failure tolerance at 10k nodes.
- Its hard new problem was **scaling diffusion stably at high resolution**,
  which SWiPe's window-parallelism addresses without inflating the global
  batch.

[^aeris]: AERIS: Argonne Earth Systems Model for Reliable and Skillful
Predictions ([arXiv:2509.13523](https://arxiv.org/abs/2509.13523)).
