...
shortTitle: 'Production Pre-Training at Scale'
...
<a class="title-slide-qr" href="https://samf.sh/talks/2026/07/14"
  ...
  {/* TODO(qr): regenerate a QR encoding samf.sh/talks/2026/07/14.
     The image below is the TPC'26 deck's QR and still points at
     .../2026/06/03 (the <span> URL and href are correct, only the
     scannable image is stale). Repo has no QR tooling; drop a new SVG
     at /talks/2026-07-14/figures/qr-slides.svg and swap the src. */}
...
| **20B**      |   512 |  5,400 | 543.6B (11.6%)  | 2.47 | advancing (auto-retry)    |
| **20B**      |   256 |  3,100 | 156.0B (3.3%)   | 2.68 | advancing                 |
...
<style>{`
  #since-tpc table th:nth-child(1), #since-tpc table td:nth-child(1) { width: 12ch; }
  #since-tpc table th:nth-child(6), #since-tpc table td:nth-child(6) { width: auto; }
...
Smith 2015[^smith-2015] / Gugger[^gugger-2017]: exponentially ramp LR over ~10% of training,
record EMA-smoothed loss, pick LR at the steepest descent (or
blow-up point / 10 as a conservative default).

See also our recent work on cross-optimizer LR scaling[^gupta-2026].
...
| Optimizer   | Loss (2B, 1k steps) | TPS/GPU   |
| ----------- | ------------------: | --------: |
| Muon        |               3.628 | 4,695     |
| **`mano`**  |           **3.631** | **7,200** |
| AdamW       |               ~3.70 | 7,200     |
...
- **Muon and `mano` tie on loss (~3.63); `mano` runs at AdamW speed**
  (7,200 vs 4,700 TPS) → **wins on wall-clock**.
...
<style>{`
  #mano table { max-width: 60ch; }
  #mano table th:nth-child(2), #mano table td:nth-child(2),
  #mano table th:nth-child(3), #mano table td:nth-child(3) { text-align: right; }
`}</style>
...
## 2B production: training loss (complete)

<figure>
    <img
        src="/talks/2026-07-14/figures/production-2b-256n.svg"
        alt="AuroraGPT-2B v2 256N production training loss over 4.67T tokens"
    />
    <figcaption>
        v2 256N, the full **4.674T-token** run (step-92,859), final loss
        **2.65**. Data:
...
<figure>
    ...
    />
    <figcaption>
        Every current production run on one axis (2B + 20B v2). Data:
...
## 20B production: training loss

<figure>
    <img
        src="/talks/2026-07-14/figures/production-20b-512n.svg"
        alt="AuroraGPT-20B v2 512N production training loss, step-5,400 / 543.6B tokens"
    />
    <figcaption>
        v2 512N (sync), step-5,400 / **543.6B tokens** (11.6%), loss
        **2.47** and still descending. Data:
...
<figure>
    <img src="/talks/2026-07-14/figures/eval-2b-overview.svg"
        alt="AuroraGPT-2B lm-eval overview: v2 256N async + v2 512N sync vs MDS SophiaG reference, per benchmark vs tokens" />
    <figcaption>
        v2 256N (async) + v2 512N (sync) vs the MDS SophiaG reference.
        Final (step-92,859, 4.67T tok): **HellaSwag 0.561 · ARC-Easy
        0.651**; plateaus ~2T tokens, still short of the MDS ceiling.
        Data:
...
## 20B eval: v2 (bf16-fixed) climbs off the noise band

<figure>
    <img src="/talks/2026-07-14/figures/eval-20b-overview.svg"
        alt="AuroraGPT-20B lm-eval overview: v1 256N, v2 256N, v2 512N sync, 2B-MDS reference, per benchmark vs training step" />
    <figcaption>
        v1 (bf16-tainted) stays at random; v2 512N climbs monotonically.
        At step-4,400 (442.9B tok): **ARC-Easy 0.693 (+41.9pp vs v1) ·
        HellaSwag 0.634 (+36.9pp)**. 20B/512N beats 2B/256N per token
        (~8× more token-efficient). Data:
...
{/*───────────────────────────────────────────────────────────────
All-production evals overlay (NEW figure: refreshed 2026-07)
───────────────────────────────────────────────────────────────*/}

<Slide id="all-evals">

## All production evals, per token

<figure>
    <img src="/talks/2026-07-14/figures/all-production-evals.svg"
        alt="All AuroraGPT production runs (2B MDS, 2B v2, 20B v2) lm-eval accuracy per benchmark vs tokens" />
    <figcaption>
        Every production run on one axis. **20B/512N is the most
        token-efficient run we have**: fewer tokens, higher accuracy
        than either 2B chain. Data:
...
- The LR-finder's early ranking said **`mano` was "safest"**; it
  wasn't. **Early-step ranking does not predict sustained stability.**
...
<style>{`
  ...
  #moe-xpu table th:nth-child(2), #moe-xpu table td:nth-child(2) { width: auto; }
`}</style>
...
```mermaid
flowchart LR
    BASE(["<span style='font-weight:700'>2B base</span><br/><i>4.67T tok</i>"])
    BASE --> CPT["<span style='font-weight:700'>CPT</span><br/><i>olmo x dolmino</i>"]
    BASE --> SFT["<span style='font-weight:700'>SFT</span><br/><i>tulu math mix</i>"]
...
- **CPT**: data-mix sweep for a second pretraining stage
- **SFT**: `checkpoint-729-hf` is now a first-class production asset
- **GRPO**: on-policy RL (TRL + vLLM-XPU), verified multi-node
...
## SFT: `tulu_math_uc_mix` on the 2B base
...
<a class="thanks-qr" href="https://samf.sh/talks/2026/07/14"
   aria-label="QR code linking to these slides">
  {/* TODO(qr): same stale-QR note as the title slide. */}
  <img src="/talks/2026-06-03/figures/qr-slides.svg"
       alt="QR code linking to samf.sh/talks/2026/07/14" />
  <span>samf.sh/talks/2026/07/14</span>
</a>
...
<style>{`
  ...
  #optimizer-cliff-detail table { font-size: 0.9em; }
`}</style>
