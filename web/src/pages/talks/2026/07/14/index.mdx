...
<Slide id="agpt-eval" size="sm">

## 2B + 20B eval: `torchtitan` vs the references

<div class="eval-two-up">

<figure>
    <img src="/talks/2026-07-14/figures/eval-2b-overview.svg"
        alt="AuroraGPT-2B lm-eval: v2 256N async + v2 512N sync vs MDS SophiaG reference, per benchmark vs tokens" />
    <figcaption>
        **2B**: v2 256N + 512N vs MDS SophiaG. Final (4.67T tok)
        HellaSwag **0.561** · ARC-Easy **0.651**; plateaus ~2T, short of
        the MDS ceiling.
    </figcaption>
</figure>
...
<figure>
    <img src="/talks/2026-07-14/figures/eval-20b-overview.svg"
        alt="AuroraGPT-20B lm-eval: v1 256N, v2 256N, v2 512N sync, 2B-MDS reference, per benchmark vs tokens" />
    <figcaption>
        **20B**: v1 (bf16) stuck at random; v2 512N climbs. step-4,400
        ARC-Easy **0.693 (+41.9pp)** · HellaSwag **0.634 (+36.9pp)**.
    </figcaption>
</figure>
...
<style>{`
  #agpt-eval .eval-two-up {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 2ch;
    align-items: start;
  }
  #agpt-eval .eval-two-up figure { margin: 0; }
  #agpt-eval .eval-two-up img { width: 100%; height: auto; }
  @media (max-width: 768px) {
    #agpt-eval .eval-two-up { grid-template-columns: 1fr; gap: 1lh; }
  }
`}</style>
...
{/*───────────────────────────────────────────────────────────────
All-production evals overlay (kept separate: cross-model, per token)
───────────────────────────────────────────────────────────────*/}
...
<figure>
    ...
    <figcaption>
        ...
        than either 2B chain (~8× per-token). Data:
        [`docs/evals`](https://github.com/saforem2/torchtitan/tree/ezpz/torchtitan/experiments/ezpz/docs/evals)
    </figcaption>
</figure>
