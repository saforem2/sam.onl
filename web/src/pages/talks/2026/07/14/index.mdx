...
<div class="ezpz-diagram-desktop">
...
<style>{`
  ...
  /* Diagram orientation swap: top-down (TB) on wide screens so the five
     target nodes stack vertically in the right column beside the code;
     left-right (LR) on narrow/mobile where a wide fan reads better than a
     tall one. Only one variant renders at a time so mermaid's interactive
     controls attach to the visible block. */
  #ezpz .ezpz-diagram-mobile { display: none; }
  @media (max-width: 768px) {
    #ezpz .ezpz-grid {
      grid-template-columns: 1fr;
      gap: 1lh;
    }
    #ezpz .ezpz-diagram-desktop { display: none; }
    #ezpz .ezpz-diagram-mobile { display: block; }
  }
...
<style>{`
  ...
  #cpt .cpt-grid > .repo-card { min-width: 0; align-self: center; }
  @media (max-width: 768px) {
