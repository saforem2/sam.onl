...
### 3. The de-stream rabbit hole (that upstream already solved)

Worth documenting because I burned real time here. The shim **forces
`stream=true`** on `/messages` (to dodge a Vertex timeout on long non-stream
requests), so a non-streaming client like rosetta gets SSE when it expects a
single JSON body. The shim therefore has to *collapse* the SSE back into one
Messages object.

I wrote that collapse logic from scratch — and then hit a second trap: rosetta's
HTTP client sends `Accept-Encoding: gzip`, so Argo **gzip-compresses the SSE**,
and my collapser was parsing gzipped bytes as text and silently failing. (`curl`
worked the whole time because it doesn't send `Accept-Encoding`.)

The lesson: **check upstream first.** Current `argo-shim` already does this
correctly — it has a `_reassemble_sse_to_message` helper *and* strips
`accept-encoding` before forwarding, so Argo never gzips in the first place. My
hand-rolled gunzip-and-collapse was solving a problem that didn't exist on a
current checkout. If you're reproducing this, just use a recent `argo-shim` and
none of it is needed.

### 4. Hermes' `custom` provider and model-name prefixes
...
### 5. The capabilities default silently drops images
...
### 6. Duplicated responses in Hermes One
...
Two of the four changes I'd made to `argo-shim` turned out to be genuinely
new and broadly useful, so they went back upstream as
[a PR][argo-pr]:
...
The other two changes I *didn't* submit, and that distinction is the useful
part:

- The **de-stream + gzip handling** was already solved upstream (see
  [Gotcha #3](#3-the-de-stream-rabbit-hole-that-upstream-already-solved)) — I'd
  reimplemented an existing feature against a stale local branch.
- The **hardcoded ALCF username** default was specific to my machine and not
  shippable; the upstreamed version is env-driven.
...
Moral: before patching someone else's tool, sync with upstream and scope your
diff to what's actually new. I'd have saved myself the entire gzip rabbit hole.
