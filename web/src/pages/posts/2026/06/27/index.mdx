...
date-modified: today
description: 'Wiring Claude Code, OpenCode, Codex, and Hermes (CLI + desktop) through a single local llm-rosetta gateway that reaches both ALCF Argo (via argo-shim) and ALCF Inference Endpoints (Sophia/Metis), making Claude, GPT, and open models reachable from any client.'
---
...
One line: creates the SSH tunnel and writes a token to
`~/.claude/settings.json`. A recent `argo-shim` already includes the
bearer-auth + user-injection support (see [Contributing
back](#contributing-back)); `uvx` fetches it from PyPI with no install step.
...
argo-ask "Claude Sonnet 4.5" say hi          # should return JSON
argo-ask "Claude Sonnet 4.5" "what's 2+2?"   # quotes are safe
```
...
```bash
uvx --from "llm-rosetta[gateway]" llm-rosetta-gateway
```
...
# An Argo model...
rosetta-ask claude-opus-4-8 say hi
...
- Any rosetta model works as the top-level `model`: swap `gpt-5.5` for
  `claude-opus-4-8`, `alcf-sophia/openai/gpt-oss-120b`, etc. (use the exact
  ids you registered in the gateway config above).
- To switch models per session without editing the config, pass them on the
  command line:

```bash
codex --model claude-opus-4-8 --model-provider rosetta
```
...
```bash
codex exec --model gpt-5.5 --model-provider rosetta "say hi"
```
...
(`gpt-5.5`, `alcf-sophia/openai/gpt-oss-120b`, …).
