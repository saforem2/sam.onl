...
> Argo's full model list and writes entries that *don't work* (Gemini 500s
> upstream; spaced display-names get mapped to the wrong provider). Manage the
> model list from the config file instead. I keep a
...
all talking to ALCF Argo through a single SSH tunnel, with every Claude and GPT
