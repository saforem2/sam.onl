...
date-modified: today
description: 'Wiring Claude Code, OpenCode, Codex, and the Hermes One desktop app through a single local llm-rosetta gateway that reaches both ALCF Argo (via argo-shim) and ALCF Inference Endpoints (Sophia/Metis), making Claude, GPT, and open models reachable from any client.'
---
...
[OpenCode][opencode], [Codex][codex], and the [Hermes One][hermes] desktop app)
without exposing ports, juggling client-specific API keys, or paying a
...
### 5. Codex (OpenAI-format, via a custom provider)
...
The result: **Claude Code, OpenCode, Codex, vtcode, and the Hermes One desktop
app** all talking to one local gateway. Argo models go through the SSH-backed shim;
