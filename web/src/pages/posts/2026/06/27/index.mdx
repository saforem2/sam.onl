...
> Argo's full model list and writes entries that _don't work_ (Gemini 500s
...
# Start/stop/restart the rosetta gateway; auto-syncs the new Argo token into
# its config on every (re)start.
rosetta-gateway() { ...  }  # start|stop|restart|status|sync-key|regen-models|log

# Export a fresh Globus access token for ALCF Inference Endpoints. Must be set
# before starting/restarting rosetta if you want alcf-sophia/alcf-metis models.
alcf-inference-token() { ... }
```
...
rosetta-gateway restart   # sync Argo token into rosetta config + reload
