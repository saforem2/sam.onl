...
<div class="flex-container icon-flex" style="flex-direction: row; margin-block-end: 1lh;">
...
> > > [!NOTE] Last.fm
> > >
> > > <p><a href="https://last.fm/user/saforem2" target="_blank">
> > >     <img
...
> > in 2019, where I used ML to build better Markov Chain Monte Carlo sampling
> > techniques for Lattice Quantum Chromodynamics ([`l2hmc-qcd`]).
> >
> > <br />
> >
...
<tr>
<td>{new Date(post.data.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
...
<style>{`
  ...
  .repo-card {
    ...
    border-radius: 0pt;
    padding: 1rem 1.1rem;
    border: 1px solid var(--background2, #dee2e6);
...
<script>{`
// Make h2 headings collapsible on the landing page
document.querySelectorAll('#main-content > h2').forEach((heading) => {
    ...
    // Copy heading styles
    for (const cls of heading.classList) summary.classList.add(cls);
    details.appendChild(summary);
    // Move all siblings until next h2 into the details
