...
def fetch_rescuetime(start: date, end: date) -> dict:
    ...
    if isinstance(categories, dict) and "rows" in categories:
        for row in categories["rows"][:10]:
            # Row format: [Rank, Time(s), People, Category]
...
def fetch_whatpulse(start: date, end: date) -> dict:
    ...
    return {
        ...
        "top_apps": [{"app": r[0], "keys": r[1], "clicks": r[2]} for r in per_app],
            ...
            lines.append(
                render_table(
                    ...
                    [
                        [
                            d["date"],
                            str(round(d["productive_hours"], 1)),
                            str(d["productivity_pulse"]),
                            d["top_category"],
                        ]
                        for d in rt["daily"]
                    ],
